...
export async function getAdmin() {
  ...
  try {
    ...
    const app = apps[0];
    const optBucket = process.env.FIREBASE_STORAGE_BUCKET || app?.options?.storageBucket || (process.env.FIREBASE_PROJECT_ID ? `${process.env.FIREBASE_PROJECT_ID}.appspot.com` : undefined);
    if (optBucket) {
