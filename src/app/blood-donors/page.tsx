...
export default function BloodDonorsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (rareOnly) params.set('rareOnly', 'true');
          ...
          <div className="mx-auto flex max-w-5xl flex-col gap-6 px-4 py-8 md:py-10">
            ...
            >
              <h1 className="text-2xl font-bold tracking-tight md:text-3xl">
                {isAr
                  ? 'المتبرعون بالدم والأنواع النادرة'
                  : 'Blood donors & rare types'}
              </h1>
              <p className="max-w-2xl text-xs text-muted-foreground md:text-sm">
                {isAr
                  ? 'مكان بسيط للتسجيل كمتبرع بالدم أو البحث عن متبرعين، بما في ذلك الأنواع النادرة. يرجى مشاركة المعلومات التي تشعر بالراحة في مشاركتها فقط.'
                  : 'A simple place to register as a blood donor or search for donors, including rare blood types. Please only share contact details you are comfortable with.'}
              </p>
            </div>
          </div>
        </section>
            ...
            <form
              ...
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  setSubmitMessage(
                    isAr
                      ? 'شكرًا — تم تسجيل بياناتك وسيتم عرضها في القائمة.'
  ...
  value={bloodType}
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2 text-[11px] md:text-xs">
                  ...
                  <span>
                    {isAr
                      ? 'حدّد هذا الخيار إذا كانت الفصيلة نادرة.'
              ...
              <div className="space-y-1">
                <label className="block text-[11px] font-semibold md:text-xs">
                  {isAr ? 'التوفّر' : 'Availability'}
                </label>
                <div className="flex flex-wrap gap-1 text-[11px] md:text-xs">
                  {(['available', 'maybe', 'unavailable'] as const).map((v) => (
                    <button
                      key={v}
                      type="button"
              ...
              <div className="md:col-span-2 flex items-center justify-between text-[11px] text-muted-foreground md:text-xs">
                <p>
                  {isAr
                    ? 'يمكنك طلب حذف بياناتك لاحقًا من إدارة خدمتي.'
              ...
              {submitMessage && (
                <div className="md:col-span-2 text-[11px] text-muted-foreground md:text-xs">
                  {submitMessage}
                </div>
              )}
            </form>
