...
import { listActiveAds, listPromotedSaleItems, type Ad } from '@/lib/ads';
...
export default function SalesFeedPage() {
  ...
  const [promoted, setPromoted] = useState<SaleItem[]>([]);
    ...
    try {
      ...
      if (needle) {
        rows = rows.filter((r: any) => {
          ...
          const inTags = tags.some((x) => String(x || '').toLowerCase().includes(needle));
          return t.includes(needle) || d.includes(needle) || tf.includes(needle) || c.includes(needle) || inTags;
        });
      }
      setItems(rows);
    } catch {
  ...
  // Load ads once (fallback)
  useEffect(() => {
    (async () => {
      setAdsLoading(true);
      try {
        const rows = await listActiveAds(5);
        setAds(rows);
      } catch {
        setAds([]);
      } finally {
  ...
  // Load promoted sale items selected by owner
  useEffect(() => {
    (async () => {
      setPromotedLoading(true);
      try {
        const rows = await listPromotedSaleItems(5);
        setPromoted(rows as any);
      } catch {
        setPromoted([]);
      } finally {
  ...
  useEffect(() => {
    const t = setTimeout(() => { void fetchItems(); }, 250);
    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="mb-4 flex justify-end">
        <div className="rounded-2xl copper-gradient p-[2px]">
          <div className="rounded-[1rem] bg-background text-foreground px-3 py-2 md:px-4 md:py-3 shadow-lg">
            {/* filters row */}
            <div className="flex flex-col gap-2 md:flex-row md:flex-wrap md:items-center md:gap-3">
              ...
              <div className="w-full md:w-auto">
                <Select value={condition} onValueChange={setCondition}>
                  <SelectTrigger className="h-10 w-full md:w-40">
                    <SelectValue placeholder={locale === 'ar' ? 'الحالة' : 'Condition'} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  className="bg-power hover:bg-powerDark text-white"
                  onClick={fetchItems}
                >
              ...
              <div className="w-full md:w-64">
                <Input
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); void fetchItems(); } }}
                  className="h-10"
                />
              </div>
            </div>
