...
export function isS3Configured(): { ok: true } | { ok: false; missing: string[] } {
  ...
  // Bucket has a default, so only mark missing if explicitly required by caller.
...
// When generating presigned URLs, the endpoint must be reachable by the browser.
// If your server can reach MinIO on a private address but users cannot, set
// `S3_PUBLIC_ENDPOINT` to the public URL (usually https) and we will sign with it.
